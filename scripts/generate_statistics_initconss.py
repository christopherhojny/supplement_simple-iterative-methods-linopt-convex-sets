...
def print_experiment_line(problem_type, log_path, initconss):
    ...
    for inst in instances:
        ...
        f = open(log_path + "/%s_prec_0.001000_corrfreq_1_initconss_%d_solver_gurobi_%s_lbopt_%d.txt" % (inst, initconss, problem_type, optval))
        ...
        f = open(log_path + "/%s_prec_0.001000_corrfreq_1_initconss_%d_solver_gurobi_%s.txt" % (inst, initconss, problem_type))
    ...
    if initconss == 1:
        conss = "upper bound"
    if avg_iter_LP == 1000:
        print("      %12s & %4.2f & %4.2f & %4s\\\\" % (conss, avg_iter_polar, avg_iter_polar_opt, "---"))
    else:
        print("      %12s & %4.2f & %4.2f & %4.2f\\\\" % (conss, avg_iter_polar, avg_iter_polar_opt, avg_iter_LP))
...
def print_new_section(problem_type):
    ...
    print("      \\midrule")
    print("      \\multicolumn{4}{@{}l}{%s:}\\\\" % problemname)
    

def print_statistics_header():
    print("% generated by generate_statistics_initconss.py")
    print("\\begin{table}[t]")
    print("  \\begin{scriptsize}")
    print("    \\caption{Comparison of iteration counts for different initial constraints in fully corrective algorithm.}")
    print("    \\label{tab:itercntInitconss}")
    print("    \\begin{tabular*}{\\textwidth}{@{}l@{\\;\\;\\extracolsep{\\fill}}rrr@{}}\\toprule")
    print("     initial constr. & standard & optimal & LP\\\\")
...
print_experiment_line("matching", log_path, 2)

print_new_section("stableset")

print_experiment_line("stableset", log_path, 1)
print_experiment_line("stableset", log_path, 2)
